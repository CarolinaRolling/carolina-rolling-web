...
export default function AngleRollForm({ partData, setPartData, vendorSuggestions, setVendorSuggestions, showVendorSuggestions, setShowVendorSuggestions, showMessage, setError }) {
  ...
  const ringCalc = useMemo(() => {
    ...
    // Weld overlap: only applies to the "cut to ring with overlap" service — adds extra material to
    // the cut so the two ends overlap at the weld joint.
  ...
  useEffect(() => {
    if (completeRings && ringCalc && !ringCalc.error) {
      ...
      setPartData(prev => ({
        ...
        _ringOverlap: cutServiceType === 'cut_to_ring_overlap' ? (parseFloat(ringOverlap) || 0) : 0,
        ...
        _baseLaborTotal: laborPerRing > 0 ? laborPerRing.toFixed(2) : prev._baseLaborTotal,
      }));
    } else {
            ...
            <div style={{ marginTop: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                ...
                <div className="form-group" style={{ margin: 0 }}>
                  ...
                  <div style={{ fontSize: '0.7rem', color: '#999', marginTop: 2 }}>Machine grab — straight material each end per ring</div>
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              ...
              {ringCalc && !ringCalc.error && (
                <div style={{ background: '#c8e6c9', borderRadius: 8, padding: 12, fontSize: '0.85rem' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8 }}>
                    ...
                    <div>
                      <div style={{ color: '#666', fontSize: '0.7rem' }}>+ Tangents ({ringCalc.tangent}" × 2)</div>
                      <div style={{ fontWeight: 600 }}>{(ringCalc.tangent * 2).toFixed(2)}"</div>
                    </div>
                    <div>
                      <div style={{ color: '#666', fontSize: '0.7rem' }}>Cut Length / Ring</div>
                      <div style={{ fontWeight: 700, color: '#1565c0' }}>{ringCalc.cutLengthPerRing.toFixed(2)}"</div>
                    </div>
                    <div>
                      <div style={{ color: '#666', fontSize: '0.7rem' }}>{ringCalc.multiSegment ? 'Segments / Ring' : 'Rings / Stock Length'}</div>
                      <div style={{ fontWeight: 700, fontSize: '1.1rem', color: '#1565c0' }}>{ringCalc.multiSegment ? ringCalc.segmentsPerRing : ringCalc.ringsPerStick}</div>
                    </div>
                  </div>
                  <div style={{ marginTop: 10, padding: '8px 0', borderTop: '1px solid #a5d6a7' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1rem', marginBottom: 4 }}>
                      <span><strong>{ringsNeeded}</strong> ring(s) needed</span>
                      <strong style={{ color: '#2e7d32', fontSize: '1.15rem' }}>ORDER: {ringCalc.sticksNeeded} × {(ringCalc.stockLength / 12).toFixed(0)}' lengths</strong>
                    </div>
                    {!ringCalc.multiSegment && ringCalc.wastePercent !== undefined && (
                      <div style={{ fontSize: '0.8rem', color: '#666' }}>
                        Drop on last stick: {((ringCalc.sticksNeeded * ringCalc.stockLength) - ((parseInt(ringsNeeded) || 1) * (ringCalc.cutLengthPerRing + ringCalc.kerf))).toFixed(1)}" — Waste: {ringCalc.wastePercent.toFixed(1)}%
                      </div>
                    )}
                  </div>
            ...
            <input className="form-input"
              ...
              onChange={async (e) => {
                ...
                if (value.length >= 1) {
                  ...
                  setPartData(prev => ({ ...prev, _vendorSearch: value, vendorId: null, supplierName: '', vendor: null })); setVendorSuggestions([]); setShowVendorSuggestions(false);
                }
              }}
