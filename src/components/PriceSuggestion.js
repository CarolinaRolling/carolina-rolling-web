 ...
 */
export default function PriceSuggestion({ partType, material, thickness, diameter, clientName, onApply }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      setLoading(true);
      try {
        const res = await getPriceSuggestion({ partType, material, thickness, diameter, clientName });
  ...
  return (
    <div style={{ marginTop: 5 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: '0.7rem', color: confColor }}>{confLabel}</span>
      ...
      {open && (
        <div style={{ marginTop: 6, background: '#fafafa', border: '1px solid #eee', borderRadius: 8, padding: 10, fontSize: '0.75rem', color: '#555' }}>
          ...
          <div style={{ color: '#777', marginBottom: 6 }}>
            Based only on quotes you actually <strong>won</strong> — recent jobs weighted more heavily.
          </div>
                ...
                {sug.samples.map((s, i) => (
                  <tr key={i} style={{ borderTop: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '3px 4px', color: '#777' }}>
                      {s.thickness ? `${s.thickness}"` : ''}{s.diameter ? ` × ${s.diameter}"` : ''}
                    </td>
