...
import { Plus, Check, X, AlertTriangle, ClipboardList, Eye, ChevronDown, ChevronUp } from 'lucide-react';
...
function TodoBar() {
  ...
  if (loading || todos.length === 0 && !showAddForm) {
    return (
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 4 }}>
        <button onClick={() => setShowAddForm(true)}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#999', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 600, fontSize: '0.85rem', color: '#f57f17' }}>
          ...
          {todos.filter(t => t.priority === 'urgent').length > 0 && (
            <span style={{ background: '#c62828', color: 'white', padding: '1px 8px', borderRadius: 10, fontSize: '0.7rem' }}>
              {todos.filter(t => t.priority === 'urgent').length} urgent
            </span>
          )}
        </div>
            ...
            return (
              <div key={todo.id} style={{
                ...
                <span style={{ fontSize: '0.9rem', flexShrink: 0 }}>
                  {typeIcons[todo.type] || colors.icon}
                </span>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {todo.description && (
                    <div style={{ fontSize: '0.8rem', color: '#666', marginTop: 2 }}>{todo.description}</div>
                  )}
                ...
                {isEstimateReview && todo.estimateId && (
                  <button onClick={() => navigate(`/estimates/${todo.estimateId}`)}
                    style={{ background: '#e3f2fd', border: '1px solid #90caf9', borderRadius: 4, padding: '4px 10px', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600, color: '#1565c0', display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                    ...
                    {denyingId === todo.id ? (
                      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                        ...
                        <button onClick={() => handleDeny(todo.id)}
                          style={{ background: '#c62828', color: 'white', border: 'none', borderRadius: 4, padding: '4px 8px', cursor: 'pointer', fontSize: '0.7rem' }}>
                          Send
                        </button>
                      ...
                      <>
                        <button onClick={() => handleAccept(todo.id)} title="Accept — pricing looks good"
                          style={{ background: '#2e7d32', color: 'white', border: 'none', borderRadius: 4, padding: '4px 10px', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 600 }}>
                          ✓ Accept
                        </button>
                        <button onClick={() => setDenyingId(todo.id)} title="Deny — needs changes"
                          style={{ background: '#c62828', color: 'white', border: 'none', borderRadius: 4, padding: '4px 10px', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 600 }}>
                          ✕ Deny
                        </button>
                      </>
                    )}
