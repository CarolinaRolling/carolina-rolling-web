...
export default function BeamRollForm({ partData, setPartData, vendorSuggestions, setVendorSuggestions, showVendorSuggestions, setShowVendorSuggestions, showMessage, setError }) {
  ...
  useEffect(() => {
    ...
    // Default to Hard Way for beams (including camber)
  ...
  const riseCalc = useMemo(() => {
    // Use INSIDE diameter (not centerline) — operators measure chord/rise on the inside
    // surface of the bent profile with a precision bar + depth gauge.
  ...
  const materialDescription = useMemo(() => {
    ...
    return parts.join(' ');
  }, [partData._beamSize, partData._customBeamSize, partData.length, partData.material, partData._materialOrigin, partData.quantity, completeRings, ringCalc]);
    ...
    <>
                        ...
                        <label style={{ display: 'flex', alignItems: 'flex-start', gap: 8, cursor: 'pointer' }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: '0.75rem', color: '#888' }}>
                              CL dia: {ringCalc.circumference ? (ringCalc.circumference / Math.PI).toFixed(3) : '?'}" (CL dia) — CL Circumference: {ringCalc.circumference?.toFixed(3)}" (pi x CL dia) — {ringCalc.ringsPerStick} ring(s)/length
                            </div>
                          </div>
                        </label>
                        <label style={{ display: 'flex', alignItems: 'flex-start', gap: 8, cursor: 'pointer' }}>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          ...
          <div className="form-group"><label className="form-label">Material Source</label><select className="form-select" value={partData.materialSource || 'customer_supplied'} onChange={(e) => setPartData(prev => ({ ...prev, materialSource: e.target.value }))}><option value="customer_supplied">Client Supplies</option><option value="we_order">We Order</option><option value="in_stock">In Stock (We Supply)</option></select></div>
        </div>
        {partData.materialSource === 'we_order' && (
          ...
          <div className="form-group" style={{ position: 'relative', marginTop: 8 }}>
            ...
            <input className="form-input" value={partData._vendorSearch !== undefined ? partData._vendorSearch : (partData.supplierName || partData.vendor?.name || '')}
              onChange={async (e) => { const value = e.target.value; setPartData(prev => ({ ...prev, _vendorSearch: value })); if (value.length >= 1) { try { const res = await searchVendors(value); setVendorSuggestions(res.data.data || []); setShowVendorSuggestions(true); } catch { setVendorSuggestions([]); } } else { setPartData(prev => ({ ...prev, _vendorSearch: value, vendorId: null, supplierName: '', vendor: null })); setVendorSuggestions([]); setShowVendorSuggestions(false); } }}
            ...
            {showVendorSuggestions && (
              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 100, background: 'white', border: '1px solid #ddd', borderRadius: 4, maxHeight: 200, overflowY: 'auto', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
                {vendorSuggestions.map(v => (<div key={v.id} style={{ padding: '8px 12px', cursor: 'pointer', borderBottom: '1px solid #eee' }} onMouseDown={() => { setPartData(prev => ({ ...prev, vendorId: v.id, supplierName: v.name, vendor: null, _vendorSearch: undefined })); setShowVendorSuggestions(false); }}><strong>{v.name}</strong>{v.contactPhone && <span style={{ fontSize: '0.8rem', color: '#666', marginLeft: 8 }}>{v.contactPhone}</span>}</div>))}
                {partData._vendorSearch && partData._vendorSearch.length >= 2 && !vendorSuggestions.some(v => v.name.toLowerCase() === (partData._vendorSearch || '').toLowerCase()) && (
                  <div style={{ padding: '8px 12px', cursor: 'pointer', background: '#e8f5e9', color: '#2e7d32', fontWeight: 600 }}
                    onMouseDown={async () => { try { const resp = await createVendor({ name: partData._vendorSearch }); if (resp.data.data) { setPartData(prev => ({ ...prev, vendorId: resp.data.data.id, supplierName: resp.data.data.name, vendor: null, _vendorSearch: undefined })); showMessage(`Vendor "${resp.data.data.name}" created`); } } catch { setError('Failed to create vendor'); } setShowVendorSuggestions(false); }}>+ Add "{partData._vendorSearch}" as new vendor</div>
                )}
              </div>
            )}
          </div>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 2fr', gap: 12 }}>
          ...
          <div className="form-group"><label className="form-label">Labor (each){completeRings && ringCalc && !ringCalc.error && parseFloat(partData._ringLaborPerUnit) > 0 && <span style={{ marginLeft: 6, fontSize: '0.72rem', color: '#1976d2', fontWeight: 600 }}>auto from rings</span>}</label><input type="number" step="any" className="form-input"
